...
class levenshtein_automaton(object):
...
# Sketchy testing application.
if __name__ == '__main__':
   import sys

   PATTERN = 'CATGATTACCCTGTTATC'
   MAXTAU = 3

   forward_matcher = levenshtein_automaton(PATTERN, MAXTAU, True)
   reverse_matcher = levenshtein_automaton(reversed(PATTERN), MAXTAU)

   with open(sys.argv[1]) as f:
      for line in f:
         end = forward_matcher.find(line.rstrip())
         if end > -1:
            start = end - reverse_matcher.find(line[end::-1])
